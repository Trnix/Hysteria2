...
SCRIPT_VERSION="1.2.0" # Incremented for info/qrcode features
...
HYSTERIA_INSTALL_VARS_FILE="${HYSTERIA_CONFIG_DIR}/install_vars.conf" # File to store installation variables for info/qrcode
...
_read_from_tty() {
    local var_name="$1"
    local prompt_str="$2"
    local default_val_display="$3" # Optional: for displaying current value hint, not for default assignment
    local actual_prompt="${BLUE}${prompt_str}${NC}"
    if [ -n "$default_val_display" ]; then
        actual_prompt="${BLUE}${prompt_str} (当前: ${default_val_display:-未设置}, 直接回车不更改): ${NC}"
        if [[ "$prompt_str" == *"密码"* && -n "$default_val_display" ]]; then # Mask password display
             actual_prompt="${BLUE}${prompt_str} (当前: ******, 直接回车不更改, 输入 'random' 生成随机密码): ${NC}"
        elif [[ "$prompt_str" == *"密码"* ]]; then # No current password, but prompt for random
             actual_prompt="${BLUE}${prompt_str} (直接回车使用随机UUID, 输入 'random' 生成随机密码): ${NC}"
        fi
    fi
...
_detect_os() {
    ...
    if [ -f /etc/os-release ]; then . /etc/os-release
        ...
        else _log_error "不支持的发行版 '$ID'."; exit 1; fi
    elif command -v apk >/dev/null 2>&1; then DISTRO_FAMILY="alpine";
    elif command -v apt-get >/dev/null 2>&1; then DISTRO_FAMILY="debian";
    else _log_error "无法确定发行版类型."; exit 1; fi

    if [[ "$DISTRO_FAMILY" == "alpine" ]]; then
        PKG_INSTALL_CMD="apk add --no-cache"; PKG_UPDATE_CMD="apk update"; INIT_SYSTEM="openrc"; SERVICE_CMD="service"
        ENABLE_CMD_PREFIX="rc-update add"; ENABLE_CMD_SUFFIX="default"; SETCAP_DEPENDENCY_PKG="libcap"
        REQUIRED_PKGS_OS_SPECIFIC="openrc"; CURRENT_HYSTERIA_SERVICE_NAME="$HYSTERIA_SERVICE_NAME_OPENRC"
    elif [[ "$DISTRO_FAMILY" == "debian" ]]; then
        export DEBIAN_FRONTEND=noninteractive; PKG_INSTALL_CMD="apt-get install -y -q"; PKG_UPDATE_CMD="apt-get update -q"
...
_is_hysteria_installed() {
    ...
    if [ -f "$HYSTERIA_INSTALL_PATH" ] && [ -f "$HYSTERIA_CONFIG_FILE" ]; then
        if [ "$INIT_SYSTEM" == "systemd" ] && [ -f "/etc/systemd/system/$CURRENT_HYSTERIA_SERVICE_NAME" ]; then return 0;
        elif [ "$INIT_SYSTEM" == "openrc" ] && [ -f "/etc/init.d/$CURRENT_HYSTERIA_SERVICE_NAME" ]; then return 0; fi
    fi; return 1
}

_install_dependencies() {
    _log_info "正在更新软件包列表 (${DISTRO_FAMILY})..."; if ! $PKG_UPDATE_CMD >/dev/null; then _log_warning "更新软件包列表失败，尝试继续..."; fi
    REQUIRED_PKGS_COMMON="wget curl git openssl lsof coreutils"; REQUIRED_PKGS="$REQUIRED_PKGS_COMMON"
    if [ -n "$REQUIRED_PKGS_OS_SPECIFIC" ]; then REQUIRED_PKGS="$REQUIRED_PKGS $REQUIRED_PKGS_OS_SPECIFIC"; fi
    if ! command -v realpath &>/dev/null && [[ "$DISTRO_FAMILY" == "debian" ]]; then _log_info "确保 realpath 命令可用 (通过 coreutils)..."
    ...
    for pkg in $REQUIRED_PKGS; do installed=false
        ...
        if $installed; then _log_info "$pkg 已安装。"; else _log_info "正在安装 $pkg..."; if ! $PKG_INSTALL_CMD "$pkg" > /dev/null; then _log_error "安装 $pkg 失败。"; exit 1; fi; fi
    done; _log_success "依赖包安装成功。"
}
...
_get_server_address() { local ipv6_ip; local ipv4_ip; _log_info "正在检测服务器公网 IP 地址..."; _log_info "尝试获取 IPv6 地址..."
    ...
    _log_info "尝试获取 IPv4 地址..."; ipv4_ip=$(curl -s -m 5 -4 https://ifconfig.me || curl -s -m 5 -4 https://ip.sb || curl -s -m 5 -4 https://api.ipify.org)
    if [ -n "$ipv4_ip" ] && [[ "$ipv4_ip" != *":"* ]]; then _log_success "检测到 IPv4 地址: $ipv4_ip"; echo "$ipv4_ip"; return; else _log_warning "未检测到 IPv4 地址或获取失败。"; fi
    _log_error "无法获取服务器公网 IP 地址 (IPv4 或 IPv6)。"; exit 1; }

_setup_hy_command() {
    _ensure_root; _log_info "正在设置 '${SCRIPT_COMMAND_NAME}' 管理命令到 /usr/local/bin/${SCRIPT_COMMAND_NAME}..."
    if [[ "$HY_SCRIPT_URL_ON_GITHUB" == *"YOUR_USERNAME"* ]]; then _log_error "HY_SCRIPT_URL_ON_GITHUB 未配置!"; _log_warning "请编辑此脚本, 将 HY_SCRIPT_URL_ON_GITHUB 变量替换为实际的 GitHub Raw URL。"; _log_warning "或手动复制: sudo cp \"${0:-${SCRIPT_FILE_BASENAME}}\" /usr/local/bin/${SCRIPT_COMMAND_NAME} && sudo chmod +x /usr/local/bin/${SCRIPT_COMMAND_NAME}"; return 1; fi
    _log_info "从 URL ($HY_SCRIPT_URL_ON_GITHUB) 下载最新版管理脚本..."; TMP_SCRIPT_DOWNLOAD_PATH=$(mktemp)
    if wget -qO "$TMP_SCRIPT_DOWNLOAD_PATH" "$HY_SCRIPT_URL_ON_GITHUB"; then
        if head -n 1 "$TMP_SCRIPT_DOWNLOAD_PATH" | grep -q -E "^#!/(usr/)?bin/(bash|sh)"; then
            if [ -f "/usr/local/bin/${SCRIPT_COMMAND_NAME}" ] && ! cmp -s "$TMP_SCRIPT_DOWNLOAD_PATH" "/usr/local/bin/${SCRIPT_COMMAND_NAME}"; then _log_info "备份现有的 /usr/local/bin/${SCRIPT_COMMAND_NAME} ..."; cp "/usr/local/bin/${SCRIPT_COMMAND_NAME}" "/usr/local/bin/${SCRIPT_COMMAND_NAME}.old.$(date +%s)"; fi
            if mv "$TMP_SCRIPT_DOWNLOAD_PATH" "/usr/local/bin/${SCRIPT_COMMAND_NAME}"; then chmod +x "/usr/local/bin/${SCRIPT_COMMAND_NAME}"; _log_success "'${SCRIPT_COMMAND_NAME}' 命令已从 URL 安装/更新到 /usr/local/bin/${SCRIPT_COMMAND_NAME}"; else _log_error "移动下载脚本失败。"; rm -f "$TMP_SCRIPT_DOWNLOAD_PATH"; fi
        else _log_error "从 URL 下载的文件似乎不是有效脚本。URL: $HY_SCRIPT_URL_ON_GITHUB"; _log_warning "下载内容开头: $(head -n 1 "$TMP_SCRIPT_DOWNLOAD_PATH")"; rm -f "$TMP_SCRIPT_DOWNLOAD_PATH"; fi
    else _log_error "从 URL 下载脚本失败。"; _log_warning "可手动操作。"; rm -f "$TMP_SCRIPT_DOWNLOAD_PATH"; fi
}
...
EOF
    ...
    _setup_hy_command
    _log_info "保存安装变量到 $HYSTERIA_INSTALL_VARS_FILE ..."
    echo "HY_PASSWORD='${PASSWORD}'" > "$HYSTERIA_INSTALL_VARS_FILE"
    echo "HY_LINK_ADDRESS='${LINK_ADDRESS}'" >> "$HYSTERIA_INSTALL_VARS_FILE"
    echo "HY_PORT='${PORT}'" >> "$HYSTERIA_INSTALL_VARS_FILE"
    echo "HY_LINK_SNI='${LINK_SNI}'" >> "$HYSTERIA_INSTALL_VARS_FILE"
    echo "HY_LINK_INSECURE='${LINK_INSECURE}'" >> "$HYSTERIA_INSTALL_VARS_FILE"
    echo "HY_SNI_VALUE='${SNI_VALUE}'" >> "$HYSTERIA_INSTALL_VARS_FILE" # Used for #fragment
    chmod 600 "$HYSTERIA_INSTALL_VARS_FILE" # Secure password
    _log_success "安装变量已保存。"
...
_do_uninstall() {
    ...
    _log_info "移除Hysteria配置: $HYSTERIA_CONFIG_DIR"; rm -rf "$HYSTERIA_CONFIG_DIR" # This also removes install_vars.conf
...
_show_config() {
    ...
    local port=$(grep -E '^listen: :[0-9]+' "$HYSTERIA_CONFIG_FILE" | sed 's/listen: ://' || echo "未知"); local password=$(awk '/^auth:/,/password:/{if(/password:/) print $2}' "$HYSTERIA_CONFIG_FILE" || echo "未知")
    local masquerade_url=$(awk '/^masquerade:/,/url:/{if(/url:/) print $2}' "$HYSTERIA_CONFIG_FILE" || echo "未知")
    echo "  监听端口: $port"; echo "  密码: $password"; echo "  伪装URL: $masquerade_url"
    if grep -q '^tls:' "$HYSTERIA_CONFIG_FILE"; then local cert_path=$(awk '/^tls:/,/cert:/{if(/cert:/)print $2}' "$HYSTERIA_CONFIG_FILE" || echo "未知"); local key_path=$(awk '/^tls:/,/key:/{if(/key:/)print $2}' "$HYSTERIA_CONFIG_FILE" || echo "未知"); echo "  TLS模式: 自定义证书"; echo "    证书路径: $cert_path"; echo "    私钥路径: $key_path";
    elif grep -q '^acme:' "$HYSTERIA_CONFIG_FILE"; then local domain=$(awk '/^acme:/,/domains:/{if(/^- /)print $2}' "$HYSTERIA_CONFIG_FILE" | sed 's/"//g' || echo "未知"); local email=$(awk '/^acme:/,/email:/{if(/email:/)print $2}' "$HYSTERIA_CONFIG_FILE" || echo "未知"); echo "  TLS模式: ACME"; echo "    域名: $domain"; echo "    邮箱: $email";
...
_change_config_interactive() {
    ...
    CURRENT_PORT=$(grep -E '^listen: :[0-9]+' "$HYSTERIA_CONFIG_FILE" | sed 's/listen: ://' || echo ""); CURRENT_PASSWORD_RAW=$(awk '/^auth:/,/password:/{if(/password:/) print $2}' "$HYSTERIA_CONFIG_FILE" || echo ""); CURRENT_MASQUERADE=$(awk '/^masquerade:/,/url:/{if(/url:/) print $2}' "$HYSTERIA_CONFIG_FILE" || echo "")
...
_show_info_link() {
    _detect_os
    if ! _is_hysteria_installed; then _log_error "Hysteria 未安装。无法显示订阅链接。"; _log_info "请使用 '${SCRIPT_COMMAND_NAME} install' 安装。"; return 1; fi
    if [ ! -f "$HYSTERIA_INSTALL_VARS_FILE" ]; then _log_error "安装信息文件 ($HYSTERIA_INSTALL_VARS_FILE) 未找到。无法生成订阅链接。"; _log_info "可能需要重新运行安装或手动配置。"; return 1; fi
    
    # Source variables silently
    . "$HYSTERIA_INSTALL_VARS_FILE"
    
    if [ -z "$HY_PASSWORD" ] || [ -z "$HY_LINK_ADDRESS" ] || [ -z "$HY_PORT" ] || [ -z "$HY_LINK_SNI" ] || [ -z "$HY_LINK_INSECURE" ] || [ -z "$HY_SNI_VALUE" ]; then
        _log_error "安装信息文件不完整。无法生成订阅链接。"
        return 1
    fi
    ...
    echo ""; _log_info "Hysteria V2 订阅链接:"
...
_show_qrcode() {
    ...
    if [ ! -f "$HYSTERIA_INSTALL_VARS_FILE" ]; then _log_error "安装信息文件 ($HYSTERIA_INSTALL_VARS_FILE) 未找到。"; return 1; fi
    . "$HYSTERIA_INSTALL_VARS_FILE"
    if [ -z "$HY_PASSWORD" ]; then _log_error "安装信息文件不完整。"; return 1; fi
...
_show_menu() {
    ...
    _log_info "如果自动安装失败或您想手动更新, 可以执行 (请确保下面的URL为最新且正确):"
