...
SCRIPT_VERSION="1.1.1" # Incremented for tty read fix
...
_read_from_tty() {
    ...
    # $2: Prompt string (optional)
    local var_name="$1"
    local prompt_str="$2"
    if [ -n "$prompt_str" ]; then
        echo -n -e "${BLUE}${prompt_str}${NC}"
    fi
...
_install_dependencies() {
    ...
    REQUIRED_PKGS_COMMON="wget curl git openssl lsof coreutils" # realpath is part of coreutils
    ...
    if [ -n "$REQUIRED_PKGS_OS_SPECIFIC" ]; then
     ...
     # Ensure realpath (from coreutils) is available before using it
    if ! command -v realpath &>/dev/null && [[ "$DISTRO_FAMILY" == "debian" ]]; then
        # On some minimal Debian/Ubuntu, coreutils might not install all symlinks or all utils by default.
        # Explicitly ensure coreutils is fully installed and realpath is available.
...
_setup_hy_command() {
    ...
    if [[ "$HY_SCRIPT_URL_ON_GITHUB" == *"YOUR_USERNAME"* || "$HY_SCRIPT_URL_ON_GITHUB" == *"YOUR_REPONAME"* || "$HY_SCRIPT_URL_ON_GITHUB" == *"YOUR_BRANCH"* ]]; then
        _log_error "脚本中的 HY_SCRIPT_URL_ON_GITHUB 未正确配置! 无法自动安装/更新 '${SCRIPT_COMMAND_NAME}' 命令。"
        _log_warning "请编辑此脚本(${0} 或您保存的文件名)，将 HY_SCRIPT_URL_ON_GITHUB 变量替换为实际的 GitHub Raw URL。"
        _log_warning "如果需要，您可以手动复制此脚本:"
        _log_warning "sudo cp \"${0:-${SCRIPT_FILE_BASENAME}}\" /usr/local/bin/${SCRIPT_COMMAND_NAME} && sudo chmod +x /usr/local/bin/${SCRIPT_COMMAND_NAME}"
        return 1
    fi
    ...
    if wget -qO "$TMP_SCRIPT_DOWNLOAD_PATH" "$HY_SCRIPT_URL_ON_GITHUB"; then
        if head -n 1 "$TMP_SCRIPT_DOWNLOAD_PATH" | grep -q -E "^#!/(usr/)?bin/(bash|sh)"; then
            # Back up existing 'hy' command if it exists and is different
            if [ -f "/usr/local/bin/${SCRIPT_COMMAND_NAME}" ] && ! cmp -s "$TMP_SCRIPT_DOWNLOAD_PATH" "/usr/local/bin/${SCRIPT_COMMAND_NAME}"; then
                 _log_info "检测到现有的 /usr/local/bin/${SCRIPT_COMMAND_NAME} 与下载版本不同，正在备份..."
                 cp "/usr/local/bin/${SCRIPT_COMMAND_NAME}" "/usr/local/bin/${SCRIPT_COMMAND_NAME}.old.$(date +%s)"
            fi
            ...
            if mv "$TMP_SCRIPT_DOWNLOAD_PATH" "/usr/local/bin/${SCRIPT_COMMAND_NAME}"; then
                ...
                rm -f "$TMP_SCRIPT_DOWNLOAD_PATH" # Clean up temp file on failure
            fi
        else
...
_do_install() {
    ...
    if _is_hysteria_installed; then
        _read_confirm_tty confirm_install "Hysteria 似乎已安装。是否继续强制安装 (可能覆盖现有配置)? [y/N]: "
        if [[ "$confirm_install" != "y" && "$confirm_install" != "Y" ]]; then _log_info "安装取消。"; exit 0; fi
    ...
    _read_from_tty TLS_TYPE "请选择 [1-2, 默认 1]: "
    TLS_TYPE=${TLS_TYPE:-1}
    ...
    case $TLS_TYPE in
        1)
            _log_info "--- 自定义证书模式 ---"
            _read_from_tty USER_CERT_PATH "请输入证书 (.crt 或 .pem) 文件绝对路径 (留空则生成自签名证书): "
            if [ -z "$USER_CERT_PATH" ]; then
                _log_info "未提供证书路径，将生成自签名证书。"
                if ! command -v openssl &> /dev/null; then _log_error "openssl 未安装，请手动运行 '$PKG_INSTALL_CMD openssl' 后重试"; exit 1; fi
                _read_from_tty SELF_SIGN_SNI "请输入用于自签名证书的伪装域名/SNI (默认 www.bing.com): "
                ...
                if ! openssl req -x509 -nodes -newkey ec:<(openssl ecparam -name prime256v1) \
                    ...
                    _log_error "自签名证书生成失败，请检查 openssl 配置！"; exit 1
                fi
                _log_success "自签名证书已生成: $CERT_PATH, $KEY_PATH"
            else
                _log_info "您提供了证书路径: $USER_CERT_PATH"
                _read_from_tty USER_KEY_PATH "请输入私钥 (.key 或 .pem) 文件绝对路径: "
                if [ -z "$USER_KEY_PATH" ]; then _log_error "私钥路径不能为空，因为您已指定了证书路径。"; exit 1; fi
                
                TMP_CERT_PATH=$(realpath "$USER_CERT_PATH" 2>/dev/null)
                TMP_KEY_PATH=$(realpath "$USER_KEY_PATH" 2>/dev/null)
                if [ ! -f "$TMP_CERT_PATH" ]; then _log_error "证书文件 '$USER_CERT_PATH' (解析为 '$TMP_CERT_PATH') 无效或不存在。"; exit 1; fi
                if [ ! -f "$TMP_KEY_PATH" ]; then _log_error "私钥文件 '$USER_KEY_PATH' (解析为 '$TMP_KEY_PATH') 无效或不存在。"; exit 1; fi
                ...
                if [ -z "$SNI_VALUE" ]; then _read_from_tty MANUAL_SNI "无法从证书自动提取CN或SAN作为SNI，请输入您希望使用的SNI: "; if [ -z "$MANUAL_SNI" ]; then _log_error "SNI 不能为空！"; exit 1; fi; SNI_VALUE="$MANUAL_SNI"; else _log_info "从证书中提取到的 SNI (CN/SAN): $SNI_VALUE"; fi
            fi;;
        2)
            _log_info "--- ACME HTTP 验证模式 ---"
            _read_from_tty DOMAIN "请输入您的域名 (例如: example.com): "; if [ -z "$DOMAIN" ]; then _log_error "域名不能为空！"; exit 1; fi
            _read_from_tty INPUT_ACME_EMAIL "请输入用于 ACME 证书申请的邮箱 (默认 $DEFAULT_ACME_EMAIL): "; ACME_EMAIL=${INPUT_ACME_EMAIL:-$DEFAULT_ACME_EMAIL}
    ...
    _read_from_tty PORT_INPUT "请输入 Hysteria 监听端口 (默认 $DEFAULT_PORT): "; PORT=${PORT_INPUT:-$DEFAULT_PORT}
    _read_from_tty PASSWORD_INPUT "请输入 Hysteria 密码 (回车则使用随机UUID): "; if [ -z "$PASSWORD_INPUT" ]; then PASSWORD=$(_generate_uuid); _log_info "使用随机密码: $PASSWORD"; else PASSWORD="$PASSWORD_INPUT"; fi
    _read_from_tty MASQUERADE_URL_INPUT "请输入伪装访问的目标URL (默认 $DEFAULT_MASQUERADE_URL): "; MASQUERADE_URL=${MASQUERADE_URL_INPUT:-$DEFAULT_MASQUERADE_URL}
...
_do_uninstall() {
    ...
    if ! _is_hysteria_installed; then
        ...
        _read_confirm_tty confirm_force_uninstall "是否仍然尝试执行标准卸载步骤? [y/N]: "
        if [[ "$confirm_force_uninstall" != "y" && "$confirm_force_uninstall" != "Y" ]]; then _log_info "卸载取消。"; exit 0; fi
    fi

    _read_confirm_tty confirm_uninstall "这将卸载 Hysteria 2 并删除所有相关配置和文件。确定要卸载吗? [y/N]: "
    ...
    if [ -f "/usr/local/bin/$SCRIPT_COMMAND_NAME" ]; then
        _read_confirm_tty confirm_remove_hy "管理命令 '$SCRIPT_COMMAND_NAME' (/usr/local/bin/$SCRIPT_COMMAND_NAME) 也被检测到。是否一并移除? [y/N]: "
    ...
    case "$action" in
        start|stop|restart) _ensure_root; _log_info "正在执行: $SERVICE_CMD $action $CURRENT_HYSTERIA_SERVICE_NAME"
            ...
            if $SERVICE_CMD "$action" "$CURRENT_HYSTERIA_SERVICE_NAME"; then _log_success "操作 '$action' 成功。"
                if [[ "$action" == "start" || "$action" == "restart" ]]; then sleep 1; $SERVICE_CMD status "$CURRENT_HYSTERIA_SERVICE_NAME" 2>/dev/null | head -n 5 || $SERVICE_CMD status "$CURRENT_HYSTERIA_SERVICE_NAME"; fi # Show brief status
...
_change_config_interactive() {
    ...
    _read_from_tty NEW_PORT "新监听端口 (当前: ${CURRENT_PORT:-未设置}, 直接回车不更改): "
    NEW_PORT=${NEW_PORT:-$CURRENT_PORT}
    _read_from_tty NEW_PASSWORD_INPUT "新密码 (当前: ${CURRENT_PASSWORD_RAW:+(已设置)}, 直接回车不更改, 输入 'random' 生成随机密码): "
    ...
    _read_from_tty NEW_MASQUERADE_URL_INPUT "新伪装URL (当前: ${CURRENT_MASQUERADE:-未设置}, 直接回车不更改): "
    ...
    if $config_changed; then _log_success "配置已更新。正在重启 Hysteria 服务..."; _control_service "restart"; rm -f "$CONFIG_BACKUP_FILE"; else _log_info "未检测到配置更改。"; rm -f "$CONFIG_BACKUP_FILE"; fi
}
...
# Root check is handled within specific functions that require it.
...
case "$ACTION" in
    ...
    logs_sys)
        _detect_os # Ensure INIT_SYSTEM is set
