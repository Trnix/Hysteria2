...
_ensure_root() { if [ "$(id -u)" -ne 0 ]; then
...
_read_from_tty() {
    ...
    if [ -n "$default_val_display" ]; then
        ...
        if [[ "$prompt_str" == *"密码"* && -n "$default_val_display" ]]; then actual_prompt="${BLUE}${prompt_str} (回车不改): ${NC}"; elif [[ "$prompt_str" == *"密码"* ]]; then actual_prompt="${BLUE}${prompt_str} (回车随机生成): ${NC}"; fi
    fi
...
_install_dependencies() {
    ...
    if ! command -v realpath &>/dev/null; then
        ...
        if ! command -v realpath &>/dev/null; then
            _log_error "realpath 命令在安装 coreutils 后仍然不可用。请检查您的系统。"
            exit 1
        fi
    fi
    ...
    for pkg in $REQUIRED_PKGS; do
        ...
        if [[ "$DISTRO_FAMILY" == "alpine" ]]; then
            ...
            if rpm -q "$pkg" >/dev/null 2>&1; then installed=true; fi
        fi
...
_setup_hy_command() {
    ...
    if ! head -n 1 "$TMP_SCRIPT_DOWNLOAD_PATH" | grep -q -E "^#!/(usr/)?bin/(bash|sh)"; then
        ...
        _log_warning "文件开头: $(head -n 1 "$TMP_SCRIPT_DOWNLOAD_PATH")"
        rm -f "$TMP_SCRIPT_DOWNLOAD_PATH"
        return 1
    fi
    local needs_update=true
    if [ -f "$installed_script_path" ]; then if cmp -s "$TMP_SCRIPT_DOWNLOAD_PATH" "$installed_script_path"; then
        _log_info "下载的脚本与已安装的 '${SCRIPT_COMMAND_NAME}' 内容相同。无需更新文件。"
        needs_update=false
    else _log_info "检测到已安装脚本与远程脚本内容不同。"; fi; else _log_info "'${SCRIPT_COMMAND_NAME}' 命令尚未安装。"; fi
    ...
    else if [ -f "$installed_script_path" ] && [ ! -x "$installed_script_path" ]; then
        chmod +x "$installed_script_path"
...
_get_remote_script_version() {
    ...
    if [ -n "$remote_version_line" ]; then echo "$remote_version_line" | awk -F'"' '/^SCRIPT_VERSION=/{print $2}'; else echo ""; fi
}
_update_hy_script() {
    ...
    if _setup_hy_command; then
        if [ -n "$REMOTE_SCRIPT_VERSION" ] && [[ "$SCRIPT_VERSION" != "$REMOTE_SCRIPT_VERSION" ]]; then
            ...
            if [ -f "/usr/local/bin/${SCRIPT_COMMAND_NAME}" ]; then new_installed_version=$(grep '^SCRIPT_VERSION=' "/usr/local/bin/${SCRIPT_COMMAND_NAME}" | awk -F'"' '{print $2}' || echo "未知"); fi
...
_get_ip_geolocation_remark() {
    ...
    *) if [ -n "$country_name_en" ]; then
    ...
    if [[ "$country_cn" == "未知地区" && -n "$country_code" ]]; then
        remark="地区${country_code} Hysteria 2-${country_code}"
    elif [[ "$country_cn" == "未知地区" ]]; then
        _log_warning "无法确定有效的国家名称或代码。将使用默认备注。"
        echo ""
        return
    else
        remark="${country_cn} Hysteria 2-${country_code}"
    fi
    _log_debug "根据地理位置生成备注: ${remark}"
    echo "$remark"
}
...
_generic_control_service() {
    ...
    if [[ "$service_type" == "hysteria" ]]; then
        ...
        _log_error "内部错误: 未知的服务类型 '$service_type'。"
        return 1
    fi

    if [[ "$INIT_SYSTEM" == "systemd" ]]; then
        current_service_name_val="$service_name_systemd_val"
        service_cmd_val="$SERVICE_CMD_SYSTEMCTL"
    elif [[ "$INIT_SYSTEM" == "openrc" ]]; then
        current_service_name_val="$service_name_openrc_val"
        service_cmd_val="$SERVICE_CMD_OPENRC"
    else
        _log_error "不支持的初始化系统: $INIT_SYSTEM"
        return 1
    ...
    if [[ "$INIT_SYSTEM" == "systemd" ]] && [ -f "/etc/systemd/system/$current_service_name_val" ]; then
        service_file_exists=true
    elif [[ "$INIT_SYSTEM" == "openrc" ]] && [ -f "/etc/init.d/$current_service_name_val" ]; then
        service_file_exists=true
    fi

    if ! $is_installed_func && ! ([[ "$action" == "enable" || "$action" == "disable" ]] && $service_file_exists); then
        if $is_installed_func; then
            :
        else
            _log_error "${service_type^} 服务未安装或服务未配置。"
            return 1
        fi
    fi
    ...
    if [[ "$INIT_SYSTEM" == "systemd" ]]; then
        ...
        _log_error "不支持的初始化系统构造命令: $INIT_SYSTEM"
        return 1
    fi
    ...
    start | stop | restart)
        _ensure_root
        _log_info "执行 (${service_type^}): $cmd_to_run"
        ...
        if [[ "$INIT_SYSTEM" == "openrc" && ("$action" == "stop" || "$action" == "restart") ]]; then
            if echo "$cmd_output" | grep -q "service .* already stopped"; then
                _log_warning "${service_type^} 服务在尝试停止时已停止。"
                if [[ "$action" == "stop" ]]; then
                    cmd_exit_code=0
                fi
            fi
            if [[ "$action" == "restart" && $cmd_exit_code -ne 0 && $(echo "$cmd_output" | grep -q "service .* already stopped") ]]; then
                _log_debug "由于服务已停止，现在尝试启动..."
                local start_cmd="$service_cmd_val $current_service_name_val start"
                cmd_output=$(eval "$start_cmd" 2>&1)
                cmd_exit_code=$?
        ...
        if [ $cmd_exit_code -eq 0 ]; then
            ...
            if [[ "$action" == "stop" && ("$service_type" == "hysteria" || "$service_type" == "mtg") ]]; then
                echo "$cmd_output" | grep -vE "Service Executable path is not absolute|a \.service file without \[Service\] section" || echo " (无特定于 $service_type 的错误信息)"
            else
                echo "$cmd_output"
            fi
            _log_warning "请检查日志:"
            echo "  输出: tail -n 30 $log_out_val"
            echo "  错误: tail -n 30 $log_err_val"
            if [ "$INIT_SYSTEM" == "systemd" ]; then
                echo "  Systemd状态: $SERVICE_CMD_SYSTEMCTL status $current_service_name_val"
                echo "  Systemd日志: journalctl -u $current_service_name_val -n 20 --no-pager"
            elif [ "$INIT_SYSTEM" == "openrc" ]; then
                echo "  OpenRC状态: $SERVICE_CMD_OPENRC $current_service_name_val status"
            fi
            return 1
        fi
        ;;
    ...
    enable)
        _ensure_root
        _log_info "启用 ${service_type^} 开机自启..."
        if $ENABLE_CMD_PREFIX "$current_service_name_val" $ENABLE_CMD_SUFFIX >/dev/null 2>&1; then
            _log_success "已启用 ${service_type^} 开机自启。"
        else
            _log_error "启用 ${service_type^} 开机自启失败。"
            return 1
        fi
        ;;
    disable)
        _ensure_root
        _log_info "禁用 ${service_type^} 开机自启..."
        local disable_cmd_ok=false
        if [[ "$INIT_SYSTEM" == "systemd" ]]; then
            if $service_cmd_val disable "$current_service_name_val" >/dev/null 2>/dev/null; then
                disable_cmd_ok=true
            fi
        elif [[ "$INIT_SYSTEM" == "openrc" ]]; then
            if rc-update del "$current_service_name_val" default >/dev/null 2>&1; then
                disable_cmd_ok=true
            fi
        fi
        if $disable_cmd_ok; then
            _log_success "已禁用 ${service_type^} 开机自启。"
        else
            _log_error "禁用 ${service_type^} 开机自启失败"
            return 1
        fi
        ;;
    *)
        _log_error "未知服务操作: $action (针对 ${service_type^})"
        return 1
        ;;
    esac
}
...
_is_hysteria_installed() {
    _detect_os
    if [ -f "$HYSTERIA_INSTALL_PATH" ] && [ -f "$HYSTERIA_CONFIG_FILE" ]; then if [ "$INIT_SYSTEM" == "systemd" ] && [ -f "/etc/systemd/system/$HYSTERIA_SERVICE_NAME_SYSTEMD" ]; then return 0; elif [ "$INIT_SYSTEM" == "openrc" ] && [ -f "/etc/init.d/$HYSTERIA_SERVICE_NAME_OPENRC" ]; then return 0; fi; fi
    return 1
}
_get_hysteria_link_params() {
    ...
    _log_debug "正从 $HYSTERIA_CONFIG_FILE 解析 Hysteria 配置以生成链接..."
    ...
    if grep -q '^\s*acme:' "$HYSTERIA_CONFIG_FILE"; then
        _log_debug "检测到 Hysteria ACME 配置。"
        ...
        if [ -z "$DOMAIN_FROM_CONFIG" ]; then
            _log_error "无法从 Hysteria 配置解析ACME域名。"
            return 1
        fi
        ...
        _log_debug "检测到 Hysteria 自定义 TLS 配置。"
        ...
        if [ ! -f "$CERT_PATH_FROM_CONFIG" ]; then
            _log_error "Hysteria 配置文件中的证书路径 '$CERT_PATH_FROM_CONFIG' 无效或文件不存在。"
            return 1
        fi
        _log_debug "Hysteria 证书路径: $CERT_PATH_FROM_CONFIG"
        _log_debug "尝试从证书提取 SNI..."
        HY_SNI_VALUE=$(openssl x509 -noout -subject -nameopt RFC2253 -in "$CERT_PATH_FROM_CONFIG" 2>/dev/null | sed -n 's/.*CN=\([^,]*\).*/\1/p')
        if [ -z "$HY_SNI_VALUE" ]; then HY_SNI_VALUE=$(openssl x509 -noout -subject -in "$CERT_PATH_FROM_CONFIG" 2>/dev/null | sed -n 's/.*CN ?= ?\([^,]*\).*/\1/p' | head -n 1 | sed 's/^[ \t]*//;s/[ \t]*$//'); fi
        ...
        if [ -z "$HY_SNI_VALUE" ]; then
            _log_warning "无法提取有效SNI(CN或SAN), 使用'sni_unknown'代替。"
            HY_SNI_VALUE="sni_unknown"
        else _log_debug "提取到 SNI: $HY_SNI_VALUE"; fi
        HY_LINK_SNI="$HY_SNI_VALUE"
        HY_LINK_ADDRESS=$(_get_server_address)
        if [ $? -ne 0 ] || [ -z "$HY_LINK_ADDRESS" ]; then
            _log_error "获取公网地址失败。"
            return 1
        fi
        HY_LINK_INSECURE="1"
    else
        _log_error "无法确定 Hysteria TLS模式。"
        return 1
    fi
    if [ -z "$HY_PORT" ] || [ -z "$HY_PASSWORD" ] || [ -z "$HY_LINK_ADDRESS" ] || [ -z "$HY_LINK_SNI" ] || [ -z "$HY_LINK_INSECURE" ] || [ -z "$HY_SNI_VALUE" ]; then
        _log_error "未能解析生成 Hysteria 链接所需的所有参数。"
        return 1
    fi
    return 0
}
_display_hysteria_link_and_qrcode() {
    ...
    if [ -n "$geo_remark" ]; then final_remark="$geo_remark"; else
        _log_debug "无法生成地理位置备注，将使用基于SNI的默认备注。"
        final_remark="Hysteria-${HY_SNI_VALUE}"
    fi
...
_show_hysteria_info_and_qrcode() {
    ...
    if ! _get_hysteria_link_params; then
        _log_error "无法从当前 Hysteria 配置生成信息。"
        return 1
    fi
    _display_hysteria_link_and_qrcode
}
_do_install_hysteria() {
    ...
    *)
    ...
    if [ -f "$HYSTERIA_INSTALL_PATH" ] && command -v "$HYSTERIA_INSTALL_PATH" &>/dev/null; then
        _log_debug "检测到已安装的 Hysteria 程序，正在检查版本..."
        ...
        if [ -n "$CURRENT_HY_VER" ] && [ "$CURRENT_HY_VER" != "unknown" ]; then
            _log_debug "当前已安装 Hysteria 版本: $CURRENT_HY_VER_RAW (规范化为: $CURRENT_HY_VER). 正在获取最新版本..."
            ...
            if [ -n "$LATEST_VER_TAG" ]; then
                ...
                _log_debug "GitHub 最新 Hysteria 版本 Tag: $LATEST_VER_TAG (规范化为: $LATEST_HY_VER_CLEAN)"
                ...
                else _log_debug "Hysteria 程序版本不一致 (最新: $LATEST_HY_VER_CLEAN, 当前: $CURRENT_HY_VER)。准备下载更新。"; fi
            else _log_warning "无法从 GitHub API 获取最新 Hysteria 版本号。将继续尝试下载以确保最新。"; fi
        else _log_warning "无法获取当前已安装 Hysteria 的版本号 (输出: '$VERSION_OUTPUT')。将继续尝试下载。"; fi
    else _log_debug "Hysteria 程序未安装或无法执行。准备下载。"; fi
    if $perform_hysteria_download; then
        if _is_hysteria_installed; then
            _log_info "准备更新 Hysteria 二进制文件，将先停止现有服务..."
            _generic_control_service "hysteria" "stop"
            sleep 1
        fi
        _log_info "下载Hysteria..."
        ARCH=$(uname -m)
        case ${ARCH} in x86_64) HYSTERIA_ARCH="amd64" ;; aarch64) HYSTERIA_ARCH="arm64" ;; armv7l) HYSTERIA_ARCH="arm" ;; *)
    ...
    if [ -f "$HYSTERIA_INSTALL_PATH" ]; then
        ...
        _log_debug "Hysteria 程序准备就绪: $HYSTERIA_INSTALL_PATH"
    else if $perform_hysteria_download; then
        _log_error "Hysteria 程序文件在尝试下载后未找到于 $HYSTERIA_INSTALL_PATH。安装中止。"
        exit 1
    else
        _log_error "Hysteria 程序未安装 (${HYSTERIA_INSTALL_PATH} 不存在) 且下载被跳过。这是一个意外情况。"
        exit 1
    fi; fi
    if [ "$TLS_TYPE" -eq 2 ]; then
        _log_debug "设置cap_net_bind_service权限(ACME)..."
        if ! command -v setcap &>/dev/null; then
            _log_warning "setcap未找到,尝试安装$SETCAP_DEPENDENCY_PKG..."
            if ! $PKG_INSTALL_CMD "$SETCAP_DEPENDENCY_PKG" >/dev/null; then _log_error "安装$SETCAP_DEPENDENCY_PKG失败."; else _log_success "$SETCAP_DEPENDENCY_PKG安装成功."; fi
        fi
        if command -v setcap &>/dev/null; then if ! setcap 'cap_net_bind_service=+ep' "$HYSTERIA_INSTALL_PATH"; then _log_error "setcap失败."; else _log_success "setcap成功."; fi; else _log_error "setcap仍不可用."; fi
    fi
...
EOF
    ...
    if [[ "$INIT_SYSTEM" == "systemd" ]]; then
        ...
        _log_debug "创建 Hysteria systemd服务..."
...
EOF
        ...
        _log_debug "创建 Hysteria OpenRC服务..."
...
EOF
    ...
    _generic_control_service "hysteria" "enable"
    _log_info "准备启动/重启 Hysteria 服务..."
    _generic_control_service "hysteria" "restart"
    sleep 2
    if _generic_control_service "hysteria" "status" >/dev/null; then _log_success "Hysteria服务已成功运行！"; else _log_error "Hysteria服务状态异常!"; fi
    _setup_hy_command
...
_do_uninstall_hysteria() {
    ...
    if [[ "$INIT_SYSTEM" == "systemd" ]]; then
        current_service_name_val="$HYSTERIA_SERVICE_NAME_SYSTEMD"
        _log_debug "禁用 Hysteria systemd服务..."
        "$SERVICE_CMD_SYSTEMCTL" disable "$current_service_name_val" >/dev/null 2>/dev/null
        _log_debug "移除 Hysteria systemd服务文件..."
        ...
        _log_debug "移除 Hysteria OpenRC服务..."
        rc-update del "$current_service_name_val" default >/dev/null 2>&1
        _log_debug "移除 Hysteria OpenRC脚本..."
        rm -f "/etc/init.d/$current_service_name_val"
    fi

    _log_debug "移除Hysteria二进制: $HYSTERIA_INSTALL_PATH"
    ...
    # 删除可能存在的二维码文件
    _log_debug "清理可能存在的二维码文件..."
    find /tmp -maxdepth 1 -type f -name "hysteria_qr_*.png" -delete 2>/dev/null
    find /tmp -maxdepth 1 -type f -name "hysteria_qr_*.txt" -delete 2>/dev/null
    find "$HOME" -maxdepth 1 -type f -name "hysteria_qr_*.png" -delete 2>/dev/null
    find "$HOME" -maxdepth 1 -type f -name "hysteria_qr_*.txt" -delete 2>/dev/null
...
_show_hysteria_config() {
    _detect_os
    if ! _is_hysteria_installed; then
        _log_error "Hysteria未安装。无配置显示。"
        return 1
    fi
    _log_info "当前Hysteria配置文件($HYSTERIA_CONFIG_FILE):"
    echo "----------------------------------------------------"
    if [ -f "$HYSTERIA_CONFIG_FILE" ]; then cat "$HYSTERIA_CONFIG_FILE"; else _log_error "配置文件不存在。"; fi
    echo "----------------------------------------------------"
    _log_info "Hysteria 配置摘要:"
    ...
    if [ -z "$masquerade_url" ]; then masquerade_url=$(awk '/^\s*masquerade:/,/url:/{if(/url:/) print $2}' "$HYSTERIA_CONFIG_FILE" || echo "未知"); fi
    echo "  监听端口: $port"
    echo "  密码: $password"
    echo "  伪装URL: $masquerade_url"
    if grep -q '^\s*tls:' "$HYSTERIA_CONFIG_FILE"; then
        local cert_path=$(grep '^\s*tls:' "$HYSTERIA_CONFIG_FILE" | sed -n 's/.*cert: \([^, }]*\).*/\1/p' || echo "未知")
        local key_path=$(grep '^\s*tls:' "$HYSTERIA_CONFIG_FILE" | sed -n 's/.*key: \([^ }]*\).*/\1/p' || echo "未知")
        echo "  TLS模式: 自定义证书"
        echo "    证书路径: $cert_path"
        echo "    私钥路径: $key_path"
    elif grep -q '^\s*acme:' "$HYSTERIA_CONFIG_FILE"; then
        local domain=$(grep -A 1 '^\s*domains:' "$HYSTERIA_CONFIG_FILE" | grep '^\s*-\s*' | sed -e 's/^\s*-\s*//' -e 's/#.*//' -e 's/[ \t]*$//' -e 's/^["'\'']//' -e 's/["'\'']$//')
        local email=$(grep -A 2 '^\s*acme:' "$HYSTERIA_CONFIG_FILE" | grep 'email:' | sed -e 's/^\s*email:\s*//' -e 's/#.*//' -e 's/[[:space:]]*$//')
        echo "  TLS模式: ACME"
        echo "    域名: $domain"
        echo "    邮箱: $email"
    else echo "  TLS模式: 未知"; fi
    echo "----------------------------------------------------"
}
_change_hysteria_config_interactive() {
    ...
    _detect_os
    if ! _is_hysteria_installed; then
        _log_error "Hysteria未安装。无法更改。"
        return 1
    fi
    _log_info "更改Hysteria配置(部分)"
    _log_warning "此功能通过awk/sed修改配置,复杂情况可能不健壮。"
    _log_warning "强烈建议备份$HYSTERIA_CONFIG_FILE。"
    _log_warning "当前支持:监听端口,密码,伪装URL。"
    _log_warning "如需更改TLS模式等重大配置, 请使用 'sudo ${SCRIPT_COMMAND_NAME} install' 命令。"
    CURRENT_PORT=$(grep -E '^\s*listen:\s*:([0-9]+)' "$HYSTERIA_CONFIG_FILE" | sed -E 's/^\s*listen:\s*://' || echo "")
    CURRENT_PASSWORD_RAW=$(grep 'password:' "$HYSTERIA_CONFIG_FILE" | head -n 1 | sed -e 's/^.*password:[[:space:]]*//' -e 's/#.*//' -e 's/[[:space:]]*$//' -e 's/["'\'']//g' || echo "")
    CURRENT_MASQUERADE=$(grep '^\s*masquerade:' "$HYSTERIA_CONFIG_FILE" | sed -n 's/.*url: \([^, }]*\).*/\1/p' || echo "")
    if [ -z "$CURRENT_MASQUERADE" ]; then CURRENT_MASQUERADE=$(awk '/^\s*masquerade:/,/url:/{if(/url:/) print $2}' "$HYSTERIA_CONFIG_FILE" || echo ""); fi
    _read_from_tty NEW_PORT "新监听端口" "$CURRENT_PORT"
    NEW_PORT=${NEW_PORT:-$CURRENT_PORT}
    _read_from_tty NEW_PASSWORD_INPUT "新密码" "$CURRENT_PASSWORD_RAW"
    NEW_PASSWORD=""
    if [ -n "$NEW_PASSWORD_INPUT" ]; then if [ "$NEW_PASSWORD_INPUT" == "random" ]; then
        NEW_PASSWORD=$(_generate_uuid)
        _log_info "生成新随机密码:$NEW_PASSWORD"
    else NEW_PASSWORD="$NEW_PASSWORD_INPUT"; fi; else NEW_PASSWORD="$CURRENT_PASSWORD_RAW"; fi
    _read_from_tty NEW_MASQUERADE_URL_INPUT "新伪装URL" "$CURRENT_MASQUERADE"
    NEW_MASQUERADE=${NEW_MASQUERADE_URL_INPUT:-$CURRENT_MASQUERADE}
    local config_changed=false
    if [ "$NEW_PORT" != "$CURRENT_PORT" ] || [ "$NEW_PASSWORD" != "$CURRENT_PASSWORD_RAW" ] || [ "$NEW_MASQUERADE" != "$CURRENT_MASQUERADE" ]; then
        CONFIG_BACKUP_FILE="${HYSTERIA_CONFIG_FILE}.bak.$(date +%s)"
        cp "$HYSTERIA_CONFIG_FILE" "$CONFIG_BACKUP_FILE"
        _log_info "配置文件备份至$CONFIG_BACKUP_FILE"
        config_changed=true
    fi
    temp_config_file=$(mktemp)
    if [ "$NEW_PORT" != "$CURRENT_PORT" ]; then
        _log_info "更改端口 '$CURRENT_PORT' -> '$NEW_PORT'..."
        sed "s/^listen: :${CURRENT_PORT}/listen: :${NEW_PORT}/" "$HYSTERIA_CONFIG_FILE" >"$temp_config_file" && mv "$temp_config_file" "$HYSTERIA_CONFIG_FILE" || {
            _log_error "更改端口失败"
            cat "$CONFIG_BACKUP_FILE" >"$HYSTERIA_CONFIG_FILE"
            rm -f "$temp_config_file" "$CONFIG_BACKUP_FILE"
            return 1
        }
    fi
    if [ "$NEW_PASSWORD" != "$CURRENT_PASSWORD_RAW" ]; then
        _log_info "更改密码..."
        awk -v new_pass="$NEW_PASSWORD" 'BEGIN{pb=0} /^auth:/{pb=1;print;next} pb&&/password:/{print "  password: " new_pass;pb=0;next} pb&&NF>0&&!/^[[:space:]]/{pb=0} {print}' "$HYSTERIA_CONFIG_FILE" >"$temp_config_file" && mv "$temp_config_file" "$HYSTERIA_CONFIG_FILE" || {
            _log_error "更改密码失败"
            cat "$CONFIG_BACKUP_FILE" >"$HYSTERIA_CONFIG_FILE"
            rm -f "$temp_config_file" "$CONFIG_BACKUP_FILE"
            return 1
    ...
    if [ "$NEW_MASQUERADE" != "$CURRENT_MASQUERADE" ]; then
        _log_info "更改伪装URL '$CURRENT_MASQUERADE' -> '$NEW_MASQUERADE'..."
        awk -v new_url="$NEW_MASQUERADE" 'BEGIN{mb=0} /^masquerade:/{mb=1;print;next} mb&&/url:/{print "    url: " new_url;mb=0;next} mb&&NF>0&&!/^[[:space:]]/{mb=0} {print}' "$HYSTERIA_CONFIG_FILE" >"$temp_config_file" && mv "$temp_config_file" "$HYSTERIA_CONFIG_FILE" || {
            _log_error "更改伪装URL失败"
            cat "$CONFIG_BACKUP_FILE" >"$HYSTERIA_CONFIG_FILE"
            rm -f "$temp_config_file" "$CONFIG_BACKUP_FILE"
            return 1
        }
...
_update_hysteria_binary() {
    ...
    _detect_os
    if ! _is_hysteria_installed; then
        _log_error "Hysteria 未安装。无法更新。"
        return 1
    fi
    _log_info "检查 Hysteria 程序更新..."
    VERSION_OUTPUT=$("$HYSTERIA_INSTALL_PATH" version 2>/dev/null)
    CURRENT_VER_RAW=$(echo "$VERSION_OUTPUT" | grep '^Version:' | awk '{print $2}')
    CURRENT_VER=$(echo "$CURRENT_VER_RAW" | sed 's#^v##')
    if [ -z "$CURRENT_VER" ] || [ "$CURRENT_VER" == "unknown" ]; then
        _log_warning "无法获取当前版本 (输出: '$VERSION_OUTPUT')。尝试下载最新。"
        CURRENT_VER="unknown"
    else _log_info "当前版本: $CURRENT_VER_RAW (规范化为: $CURRENT_VER)"; fi
    _log_info "获取最新版本号..."
    LATEST_VER_TAG=$(curl -s --connect-timeout 5 "https://api.github.com/repos/apernet/hysteria/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    if [ -z "$LATEST_VER_TAG" ]; then
        _log_warning "无法从 GitHub API 获取版本号。尝试下载标记为 'latest' 的版本。"
        ...
        if [[ "$CURRENT_VER" == "$LATEST_VER_CLEAN" && "$CURRENT_VER" != "unknown" ]]; then
            _log_success "当前已是最新版本 ($CURRENT_VER_RAW)。"
            return 0
        fi
    fi
    _log_info "下载 Hysteria (目标版本: ${LATEST_VER_CLEAN:-latest}) ..."
    ARCH=$(uname -m)
    case ${ARCH} in x86_64) HYSTERIA_ARCH="amd64" ;; aarch64) HYSTERIA_ARCH="arm64" ;; armv7l) HYSTERIA_ARCH="arm" ;; *)
        _log_error "不支持架构: ${ARCH}"
        return 1
    ...
    if ! wget -qO "$TMP_HY_DOWNLOAD" "$DOWNLOAD_URL"; then
        ...
        if ! wget -qO "$TMP_HY_DOWNLOAD" "https://download.hysteria.network/app/latest/hysteria-linux-${HYSTERIA_ARCH}"; then
            _log_error "下载 Hysteria 失败! (URL: $DOWNLOAD_URL 和备用地址)"
            rm -f "$TMP_HY_DOWNLOAD"
            _generic_control_service "hysteria" "start" &>/dev/null || true
            return 1
        fi
    fi
    ...
    DOWNLOADED_VER_OUTPUT=$("$TMP_HY_DOWNLOAD" version 2>/dev/null)
    DOWNLOADED_VER_RAW=$(echo "$DOWNLOADED_VER_OUTPUT" | grep '^Version:' | awk '{print $2}')
    DOWNLOADED_VER=$(echo "$DOWNLOADED_VER_RAW" | sed 's#^v##')
    if [[ -n "$DOWNLOADED_VER" && "$DOWNLOADED_VER" == "$CURRENT_VER" && "$CURRENT_VER" != "unknown" ]]; then
        _log_info "下载版本($DOWNLOADED_VER_RAW)与当前相同。取消更新。"
        rm -f "$TMP_HY_DOWNLOAD"
        _generic_control_service "hysteria" "start" &>/dev/null || true
        return 0
    elif [[ -n "$DOWNLOADED_VER" ]]; then _log_info "下载版本为: $DOWNLOADED_VER_RAW (规范化为: $DOWNLOADED_VER)"; else _log_warning "无法获取下载文件版本号 (输出: '$DOWNLOADED_VER_OUTPUT')。"; fi
    _log_info "准备替换二进制文件..."
    if mv "$TMP_HY_DOWNLOAD" "$HYSTERIA_INSTALL_PATH"; then
        ...
        _generic_control_service "hysteria" "start"
        return 0
    else
        _log_error "替换二进制文件失败。"
        rm -f "$TMP_HY_DOWNLOAD"
        _log_info "尝试重启旧服务..."
        _generic_control_service "hysteria" "start" &>/dev/null || true
        return 1
...
_is_mtg_installed() {
    _detect_os
    if [ -f "$MTG_INSTALL_PATH" ] && [ -f "$MTG_CONFIG_FILE" ]; then if [ "$INIT_SYSTEM" == "systemd" ] && [ -f "/etc/systemd/system/$MTG_SERVICE_NAME_SYSTEMD" ]; then return 0; elif [ "$INIT_SYSTEM" == "openrc" ] && [ -f "/etc/init.d/$MTG_SERVICE_NAME_OPENRC" ]; then return 0; fi; fi
    return 1
}
...
_download_mtg_binary() {
    ...
    *)
        _log_error "MTG: 不支持的系统架构: ${ARCH} (版本 ${expected_version})"
        return 1
        ;;
    esac
    ...
    if ! wget -qO "$tmp_archive" "$download_url"; then
        _log_error "下载 MTG 失败。URL: $download_url"
        rm -f "$tmp_archive"
        return 1
    fi
    ...
    if ! tar -xzf "$tmp_archive" -C "$tmp_extract_dir"; then
        _log_error "解压 MTG 归档文件失败: $tmp_archive"
        rm -f "$tmp_archive"
    ...
    if [ -f "$extracted_binary_path" ]; then
        if ! mv "$extracted_binary_path" "$MTG_INSTALL_PATH"; then
            _log_error "移动 MTG 二进制文件到 $MTG_INSTALL_PATH 失败。"
            rm -f "$tmp_archive"
            rm -rf "$tmp_extract_dir"
            return 1
        fi
        chmod +x "$MTG_INSTALL_PATH"
        _log_success "MTG 二进制文件已安装/更新到 $MTG_INSTALL_PATH (版本 ${expected_version})"
    else
        _log_error "在解压的归档中未找到 MTG 二进制文件 (尝试路径: $extracted_binary_path)。"
        rm -f "$tmp_archive"
        rm -rf "$tmp_extract_dir"
        return 1
    fi
...
_generate_mtg_config() {
    ...
    if [ -z "$mtg_secret" ] || [[ "$mtg_secret" != ee* ]]; then
        _log_error "MTProto 密钥生成失败或格式不正确 (需要 'ee' 开头)。请确保 MTG 程序工作正常并能生成 FakeTLS 密钥。输出: $mtg_secret"
        return 1
    fi
...
_get_mtg_link_info() {
    ...
    if [ -z "$MTG_LINK_VAR" ]; then
        _log_error "无法从 'mtg access' 输出中解析 tg_url。输出内容:\n$mtg_access_json"
        return 1
    fi
...
_display_mtg_link_and_qrcode() {
    ...
    if command -v "$MTG_INSTALL_PATH" &>/dev/null && [ -f "$MTG_CONFIG_FILE" ]; then
        ...
        if [ -n "$mtg_tme_link" ]; then
            echo ""
            _log_info "MTProto (t.me) 链接:"
            echo -e "${GREEN}${mtg_tme_link}${NC}"
        fi
    fi
    echo ""
    if command -v qrencode &>/dev/null; then
...
_do_install_mtp() {
    ...
    _install_dependencies
    if ! _download_mtg_binary "$MTG_TARGET_VERSION" "true"; then
        _log_error "MTG 二进制文件下载失败。中止安装。"
        return 1
    fi
    if ! _generate_mtg_config; then
        _log_error "MTG 配置文件生成失败。中止安装。"
        return 1
    fi
    if ! _create_mtg_service_file; then
        _log_error "MTG 服务文件创建失败。中止安装。"
        return 1
    fi
    _log_info "--- MTProto (mtg) 安装阶段结束 ---"
    _generic_control_service "mtg" "enable"
    _log_info "准备启动/重启 MTProto 代理服务..."
    _generic_control_service "mtg" "restart"
    sleep 2
    if _generic_control_service "mtg" "status" >/dev/null; then _log_success "MTProto 代理 (mtg) 服务已成功运行！"; else _log_error "MTProto 代理 (mtg) 服务状态异常!"; fi
    _log_success "MTProto 代理 (mtg) 安装配置完成！"
    echo "------------------------------------------------------------------------"
    _show_mtg_info_and_qrcode
    echo "------------------------------------------------------------------------"
    _show_management_commands_hint
...
_do_uninstall_mtp() {
    ...
    if [[ "$INIT_SYSTEM" == "systemd" ]]; then
        ...
        "$SERVICE_CMD_SYSTEMCTL" disable "$current_service_name_val" >/dev/null 2>/dev/null
        _log_debug "移除 MTG systemd 服务文件: $current_service_name_val"
        rm -f "/etc/systemd/system/$current_service_name_val"
        find "/etc/systemd/system/" -name "$current_service_name_val" -delete 2>/dev/null
        "$SERVICE_CMD_SYSTEMCTL" daemon-reload 2>/dev/null
        "$SERVICE_CMD_SYSTEMCTL" reset-failed "$current_service_name_val" >/dev/null 2>/dev/null || true
        ...
        current_service_name_val="$MTG_SERVICE_NAME_OPENRC"
        _log_debug "移除 MTG OpenRC 服务脚本: $current_service_name_val"
        rc-update del "$current_service_name_val" default >/dev/null 2>&1
        rm -f "/etc/init.d/$current_service_name_val"
    fi
    _log_debug "移除 MTProto (mtg) 二进制文件: $MTG_INSTALL_PATH"
...
_edit_mtg_config() {
    ...
    if $EDITOR "$MTG_CONFIG_FILE"; then
        _log_info "编辑完成。请使用 'sudo $SCRIPT_COMMAND_NAME restart_mtp' 或 'sudo $SCRIPT_COMMAND_NAME re_mtp' 重启服务以应用更改。"
...
_show_menu() {
    ...
    echo "  up           - 更新 Hysteria, MTG 程序和此管理脚本"
...
# --- 在Script Setup部分之后，添加交互式选择函数 ---

# --- Interactive Service Selection Function ---
_interactive_service_selection() {
    local action="$1"
    local action_desc="$2"
    ...
    1)
        ...
        "stop") _generic_control_service "hysteria" "stop" ;;
        "restart") _generic_control_service "hysteria" "restart" ;;
        "status") _generic_control_service "hysteria" "status" ;;
        "enable") _generic_control_service "hysteria" "enable" ;;
        "disable") _generic_control_service "hysteria" "disable" ;;
        "info") _show_hysteria_info_and_qrcode ;;
        "logs")
            _log_info "按 CTRL+C 退出 (Hysteria 日志 $LOG_FILE_HYSTERIA_OUT)。"
            tail -f "$LOG_FILE_HYSTERIA_OUT"
            ;;
        "logs_err")
            _log_info "按 CTRL+C 退出 (Hysteria 错误日志 $LOG_FILE_HYSTERIA_ERR)。"
            tail -f "$LOG_FILE_HYSTERIA_ERR"
            ;;
    ...
    2)
        ...
        case "$action" in
        "install") _do_install_mtp ;;
        "start") _generic_control_service "mtg" "start" ;;
        "stop") _generic_control_service "mtg" "stop" ;;
        "restart") _generic_control_service "mtg" "restart" ;;
        "status") _generic_control_service "mtg" "status" ;;
        "enable") _generic_control_service "mtg" "enable" ;;
        "disable") _generic_control_service "mtg" "disable" ;;
        "info") _show_mtg_info_and_qrcode ;;
        "logs")
            if [[ "$INIT_SYSTEM" == "openrc" ]]; then
                _log_info "按 CTRL+C 退出 (MTG 日志 $LOG_FILE_MTG_OUT)。"
                tail -f "$LOG_FILE_MTG_OUT"
            elif [[ "$INIT_SYSTEM" == "systemd" ]]; then
                _log_info "对于 systemd 系统, 将显示 MTProto 服务的 systemd 日志。"
                journalctl -u "$MTG_SERVICE_NAME_SYSTEMD" -f --no-pager
            fi
            ;;
        "logs_err")
            if [[ "$INIT_SYSTEM" == "openrc" ]]; then
                _log_info "按 CTRL+C 退出 (MTG 错误日志 $LOG_FILE_MTG_ERR)。"
                tail -f "$LOG_FILE_MTG_ERR"
            elif [[ "$INIT_SYSTEM" == "systemd" ]]; then
                _log_info "对于 systemd 系统, 将显示 MTProto 服务的 systemd 日志。"
                journalctl -u "$MTG_SERVICE_NAME_SYSTEMD" -f --no-pager
            fi
...
if [[ -n "$TARGET" ]]; then
    ...
    "sp mtp") _generic_control_service "mtg" "stop" ;;
    "re hy") _generic_control_service "hysteria" "restart" ;;
    "re mtp") _generic_control_service "mtg" "restart" ;;
    "s hy") _generic_control_service "hysteria" "status" ;;
    "s mtp") _generic_control_service "mtg" "status" ;;
    "e hy") _generic_control_service "hysteria" "enable" ;;
    "e mtp") _generic_control_service "mtg" "enable" ;;
    "d hy") _generic_control_service "hysteria" "disable" ;;
    "d mtp") _generic_control_service "mtg" "disable" ;;
    "n hy") _show_hysteria_info_and_qrcode ;;
    "n mtp") _show_mtg_info_and_qrcode ;;
    "l hy")
        _log_info "按 CTRL+C 退出 (Hysteria 日志 $LOG_FILE_HYSTERIA_OUT)。"
        tail -f "$LOG_FILE_HYSTERIA_OUT"
        ;;
    ...
    "stop hy") _generic_control_service "hysteria" "stop" ;;
    "stop mtp") _generic_control_service "mtg" "stop" ;;
    "restart hy" | "rs hy") _generic_control_service "hysteria" "restart" ;;
    "restart mtp" | "rs mtp") _generic_control_service "mtg" "restart" ;;
    "status hy") _generic_control_service "hysteria" "status" ;;
    "status mtp") _generic_control_service "mtg" "status" ;;
    "enable hy" | "en hy") _generic_control_service "hysteria" "enable" ;;
    "enable mtp" | "en mtp") _generic_control_service "mtg" "enable" ;;
    "disable hy" | "dis hy") _generic_control_service "hysteria" "disable" ;;
    "disable mtp" | "dis mtp") _generic_control_service "mtg" "disable" ;;
    "info hy" | "nfo hy") _show_hysteria_info_and_qrcode ;;
    "info mtp" | "nfo mtp") _show_mtg_info_and_qrcode ;;
    "logs hy" | "log hy")
        _log_info "按 CTRL+C 退出 (Hysteria 日志 $LOG_FILE_HYSTERIA_OUT)。"
        tail -f "$LOG_FILE_HYSTERIA_OUT"
        ;;
    ...
    "logs_err mtp" | "loge mtp")
        ...
        ;;
    *)
        _log_error "未知命令组合: $ACTION $TARGET"
        _show_menu
        exit 1
        ;;
    esac
...
s) _interactive_service_selection "status" "查看状态" ;;
e) _interactive_service_selection "enable" "启用" ;;
d) _interactive_service_selection "disable" "禁用" ;;
n) _interactive_service_selection "info" "显示信息" ;;
l) _interactive_service_selection "logs" "查看日志" ;;
le) _interactive_service_selection "logs_err" "查看错误日志" ;;

# 保留原命令兼容性
run | start) _interactive_service_selection "start" "启动" ;;
stop) _interactive_service_selection "stop" "停止" ;;
rs | restart) _interactive_service_selection "restart" "重启" ;;
st | status) _interactive_service_selection "status" "查看状态" ;;
en | enable) _interactive_service_selection "enable" "启用" ;;
dis | disable) _interactive_service_selection "disable" "禁用" ;;
nfo | info) _interactive_service_selection "info" "显示信息" ;;
log | logs) _interactive_service_selection "logs" "查看日志" ;;
loge | logs_err) _interactive_service_selection "logs_err" "查看错误日志" ;;
...
up) _do_update ;;
v)
    echo "$SCRIPT_COMMAND_NAME 管理脚本 v$SCRIPT_VERSION ($SCRIPT_DATE)"
    echo "脚本文件: $SCRIPT_FILE_BASENAME"
