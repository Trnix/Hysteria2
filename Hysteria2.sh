...
SCRIPT_VERSION="1.3.0" # Incremented for update/fix features
...
HYSTERIA_INSTALL_VARS_FILE="${HYSTERIA_CONFIG_DIR}/install_vars.conf" # May become legacy, but keep for now
...
_setup_hy_command() {
    _ensure_root; _log_info "设置'${SCRIPT_COMMAND_NAME}'命令到/usr/local/bin/${SCRIPT_COMMAND_NAME}..."; if [[ "$HY_SCRIPT_URL_ON_GITHUB" == *"YOUR_USERNAME"* ]]; then _log_error "HY_SCRIPT_URL_ON_GITHUB未配置!"; _log_warning "请编辑脚本设置URL或手动复制: sudo cp \"${0:-${SCRIPT_FILE_BASENAME}}\" /usr/local/bin/${SCRIPT_COMMAND_NAME} && sudo chmod +x /usr/local/bin/${SCRIPT_COMMAND_NAME}"; return 1; fi
    _log_info "从URL($HY_SCRIPT_URL_ON_GITHUB)下载..."; TMP_SCRIPT_DOWNLOAD_PATH=$(mktemp); if wget -qO "$TMP_SCRIPT_DOWNLOAD_PATH" "$HY_SCRIPT_URL_ON_GITHUB"; then if head -n 1 "$TMP_SCRIPT_DOWNLOAD_PATH" | grep -q -E "^#!/(usr/)?bin/(bash|sh)"; then if [ -f "/usr/local/bin/${SCRIPT_COMMAND_NAME}" ] && ! cmp -s "$TMP_SCRIPT_DOWNLOAD_PATH" "/usr/local/bin/${SCRIPT_COMMAND_NAME}"; then _log_info "备份现有命令..."; cp "/usr/local/bin/${SCRIPT_COMMAND_NAME}" "/usr/local/bin/${SCRIPT_COMMAND_NAME}.old.$(date +%s)"; fi; if mv "$TMP_SCRIPT_DOWNLOAD_PATH" "/usr/local/bin/${SCRIPT_COMMAND_NAME}"; then chmod +x "/usr/local/bin/${SCRIPT_COMMAND_NAME}"; _log_success "'${SCRIPT_COMMAND_NAME}'命令已安装/更新."; else _log_error "移动下载脚本失败."; rm -f "$TMP_SCRIPT_DOWNLOAD_PATH"; fi; else _log_error "下载内容非有效脚本. URL: $HY_SCRIPT_URL_ON_GITHUB"; _log_warning "开头: $(head -n 1 "$TMP_SCRIPT_DOWNLOAD_PATH")"; rm -f "$TMP_SCRIPT_DOWNLOAD_PATH"; fi; else _log_error "下载脚本失败."; rm -f "$TMP_SCRIPT_DOWNLOAD_PATH"; fi
}

# --- Function to parse current config for link generation ---
_get_link_params_from_config() {
    # Output variables: Needs to export or use subshell return
    ...
    if [ ! -f "$HYSTERIA_CONFIG_FILE" ]; then
        _log_error "配置文件 $HYSTERIA_CONFIG_FILE 未找到。"
        return 1
    fi

    HY_PORT=$(grep -E '^listen: :[0-9]+' "$HYSTERIA_CONFIG_FILE" | sed 's/listen: ://' || echo "")
    HY_PASSWORD=$(awk '/^auth:/,/password:/{if(/password:/) print $2}' "$HYSTERIA_CONFIG_FILE" || echo "")

    if grep -q '^acme:' "$HYSTERIA_CONFIG_FILE"; then
        # ACME Mode
        DOMAIN_FROM_CONFIG=$(awk '/^acme:/,/domains:/{if(/^- /)print $2}' "$HYSTERIA_CONFIG_FILE" | sed 's/["'\'']//g' | head -n 1 || echo "")
        ...
        HY_LINK_INSECURE="0"
        HY_SNI_VALUE="$DOMAIN_FROM_CONFIG" # For fragment
    elif grep -q '^tls:' "$HYSTERIA_CONFIG_FILE"; then
        # Custom TLS Mode
        CERT_PATH_FROM_CONFIG=$(awk '/^tls:/,/cert:/{if(/cert:/)print $2}' "$HYSTERIA_CONFIG_FILE" || echo "")
        if [ -z "$CERT_PATH_FROM_CONFIG" ] || [ ! -f "$CERT_PATH_FROM_CONFIG" ]; then _log_error "无法解析或找到证书路径 '$CERT_PATH_FROM_CONFIG'。"; return 1; fi
        
        # Try extracting SNI from cert again
        HY_SNI_VALUE=$(openssl x509 -noout -subject -in "$CERT_PATH_FROM_CONFIG" 2>/dev/null | grep -o 'CN=[^,]*' | cut -d= -f2 | tr -d ' ' | head -n 1)
        if [ -z "$HY_SNI_VALUE" ]; then HY_SNI_VALUE=$(openssl x509 -noout -text -in "$CERT_PATH_FROM_CONFIG" 2>/dev/null | grep 'DNS:' | sed 's/DNS://g' | tr -d ' ' | cut -d, -f1 | head -n 1); fi
        if [ -z "$HY_SNI_VALUE" ]; then _log_warning "无法从证书'$CERT_PATH_FROM_CONFIG'提取SNI, 链接可能不完整。"; HY_SNI_VALUE=" sni_unknown"; fi # Use placeholder if failed

        HY_LINK_SNI="$HY_SNI_VALUE"
        HY_LINK_ADDRESS=$(_get_server_address) # Re-fetch current IP
        HY_LINK_INSECURE="1" # Assume insecure needed for custom certs
    else
        _log_error "无法确定TLS模式 (ACME或自定义)。"
        return 1
    fi
...
_do_install() {
    # ... (Installation logic as in previous version, including user prompts with _read_from_tty) ...
    # IMPORTANT CHANGE: Remove the saving of variables to HYSTERIA_INSTALL_VARS_FILE at the end.
    # Instead, rely on parsing config.yaml for `hy info` and `hy qrcode`.
    # Just ensure all necessary installation steps complete.
...
EOF
    ...
    # --- Final Setup and Info Display ---
    _setup_hy_command # Attempt to install/update the 'hy' command
    
    # Display info based on current config, don't save vars to file anymore
    if _get_link_params_from_config; then
        ...
        echo "服务器地址: $HY_LINK_ADDRESS"; echo "端口: $HY_PORT"; echo "密码: $HY_PASSWORD"; echo "SNI: $HY_LINK_SNI"
        echo "伪装目标站点: $MASQUERADE_URL"; # This one comes from input, might differ if edited manually
        if grep -q '^acme:' "$HYSTERIA_CONFIG_FILE"; then echo "TLS模式: 2 (ACME)"; echo "  域名: $DOMAIN"; echo "  邮箱: $ACME_EMAIL";
        elif grep -q '^tls:' "$HYSTERIA_CONFIG_FILE"; then echo "TLS模式: 1 (Custom)"; echo "  证书路径: $CERT_PATH"; echo "  私钥路径: $KEY_PATH"; fi
        echo "客户端insecure(0=false,1=true): $HY_LINK_INSECURE"; echo "------------------------------------------------------------------------"; echo -e "${YELLOW}订阅链接(V2):${NC}"; echo "$SUBSCRIPTION_LINK"; echo "------------------------------------------------------------------------"
        ...
        _log_error "安装完成，但无法从配置文件生成最终的订阅链接信息。"
    fi
...
_do_uninstall() {
    ...
    _log_info "移除Hysteria配置: $HYSTERIA_CONFIG_DIR"; rm -rf "$HYSTERIA_CONFIG_DIR" # Removes config and certs dir and install_vars if exists
    _log_info "移除Hysteria日志: $LOG_FILE_OUT, $LOG_FILE_ERR"; rm -f "$LOG_FILE_OUT" "$LOG_FILE_ERR"
    
    # Ask about qrencode
    if command -v qrencode &>/dev/null; then
       _read_confirm_tty confirm_remove_qrencode "工具 'qrencode' 已安装。是否卸载它? [y/N]: "
       if [[ "$confirm_remove_qrencode" == "y" || "$confirm_remove_qrencode" == "Y" ]]; then
           _log_info "正在卸载 qrencode..."
           if $PKG_REMOVE_CMD qrencode >/dev/null; then _log_success "qrencode 已卸载。"; else _log_error "卸载 qrencode 失败。"; fi
       fi
    fi

    # Ask about hy command
...
_control_service() {
    # ... (Function remains mostly the same as previous version, ensure root check where needed) ...
...
_show_config() {
    ...
    # Improved parsing using grep and simplified awk/sed
    local port=$(grep -E '^\s*listen:\s*:([0-9]+)' "$HYSTERIA_CONFIG_FILE" | sed -E 's/^\s*listen:\s*://' || echo "未知")
    local password=$(grep -A 2 '^\s*auth:' "$HYSTERIA_CONFIG_FILE" | grep 'password:' | sed -E 's/^\s*password:\s*//' || echo "未知")
    local masquerade_url=$(grep -A 3 '^\s*masquerade:' "$HYSTERIA_CONFIG_FILE" | grep 'url:' | sed -E 's/^\s*url:\s*//' || echo "未知")
    
    echo "  监听端口: $port"; echo "  密码: $password"; echo "  伪装URL: $masquerade_url"
    
    if grep -q '^\s*tls:' "$HYSTERIA_CONFIG_FILE"; then 
        local cert_path=$(grep -A 2 '^\s*tls:' "$HYSTERIA_CONFIG_FILE" | grep 'cert:' | sed -E 's/^\s*cert:\s*//' || echo "未知")
        local key_path=$(grep -A 2 '^\s*tls:' "$HYSTERIA_CONFIG_FILE" | grep 'key:' | sed -E 's/^\s*key:\s*//' || echo "未知")
        echo "  TLS模式: 自定义证书"; echo "    证书路径: $cert_path"; echo "    私钥路径: $key_path";
    elif grep -q '^\s*acme:' "$HYSTERIA_CONFIG_FILE"; then 
        local domain=$(grep -A 2 '^\s*acme:' "$HYSTERIA_CONFIG_FILE" | grep -- '- ' | sed -E 's/^\s*-\s*// ; s/["'\'']//g' | head -n 1 || echo "未知")
        local email=$(grep -A 2 '^\s*acme:' "$HYSTERIA_CONFIG_FILE" | grep 'email:' | sed -E 's/^\s*email:\s*//' || echo "未知")
        echo "  TLS模式: ACME"; echo "    域名: $domain"; echo "    邮箱: $email";
    else 
        echo "  TLS模式: 未知"; 
    fi; 
    echo "----------------------------------------------------"
...
_change_config_interactive() {
    ...
    _log_info "更改Hysteria配置(部分)"; _log_warning "此功能通过awk修改配置,复杂情况可能不健壮。"; _log_warning "强烈建议备份$HYSTERIA_CONFIG_FILE。"; _log_warning "当前支持:监听端口,密码,伪装URL。"
    
    # Use the improved parsing logic from _show_config to get current values
    CURRENT_PORT=$(grep -E '^\s*listen:\s*:([0-9]+)' "$HYSTERIA_CONFIG_FILE" | sed -E 's/^\s*listen:\s*://' || echo "")
    CURRENT_PASSWORD_RAW=$(grep -A 2 '^\s*auth:' "$HYSTERIA_CONFIG_FILE" | grep 'password:' | sed -E 's/^\s*password:\s*//' || echo "")
    CURRENT_MASQUERADE=$(grep -A 3 '^\s*masquerade:' "$HYSTERIA_CONFIG_FILE" | grep 'url:' | sed -E 's/^\s*url:\s*//' || echo "")
    ...
    # Use simpler sed for replacement now that current values are known
    ...
    if $config_changed; then 
        _log_success "配置更新。重启服务..."; 
        _control_service "restart"; 
        # No need to update install_vars.conf as info/qrcode parse config directly
...
_show_info_link() {
    _detect_os
    if ! _is_hysteria_installed; then _log_error "Hysteria 未安装。无法显示订阅链接。"; return 1; fi
    
    if ! _get_link_params_from_config; then # Call function to parse config and set global HY_* vars
        _log_error "无法从当前配置生成订阅链接信息。"
        return 1
    fi
...
_show_qrcode() {
    _detect_os
    if ! _is_hysteria_installed; then _log_error "Hysteria 未安装。"; return 1; fi

    if ! _get_link_params_from_config; then
        _log_error "无法从当前配置生成二维码信息。"
        return 1
    fi
...
_update_hysteria_binary() {
    ...
    _log_info "检查 Hysteria 更新..."
    CURRENT_VER=$("$HYSTERIA_INSTALL_PATH" version 2>/dev/null | awk '{print $3}')
    if [ -z "$CURRENT_VER" ]; then
        _log_warning "无法获取当前 Hysteria 版本。将尝试直接下载最新版。"
        CURRENT_VER="unknown"
    else
         _log_info "当前版本: $CURRENT_VER"
    fi
    ...
    if [ -z "$LATEST_VER_TAG" ]; then
         _log_warning "无法从 GitHub API 获取最新版本号。将尝试直接下载 'latest'。"
         LATEST_VER="latest (unknown tag)"
    else
         LATEST_VER=$LATEST_VER_TAG
         _log_info "最新版本: $LATEST_VER"
    fi

    if [[ "$CURRENT_VER" == "$LATEST_VER" && "$CURRENT_VER" != "unknown" ]]; then
        _log_success "当前已是最新版本 ($CURRENT_VER)。"
        return 0
    fi

    _log_info "正在下载 Hysteria $LATEST_VER ..."
    ...
    DOWNLOAD_URL="https://github.com/apernet/hysteria/releases/latest/download/hysteria-linux-${HYSTERIA_ARCH}" # Always try latest download URL
    
    if ! wget -qO "$TMP_HY_DOWNLOAD" "$DOWNLOAD_URL"; then
         _log_error "下载最新 Hysteria 失败! URL: $DOWNLOAD_URL"
    ...
    if ! file "$TMP_HY_DOWNLOAD" | grep -q "executable"; then
         _log_error "下载的文件似乎不是可执行文件。"
         rm -f "$TMP_HY_DOWNLOAD"
         return 1
    fi
    chmod +x "$TMP_HY_DOWNLOAD"
    
    # Check downloaded version if possible
    DOWNLOADED_VER=$("$TMP_HY_DOWNLOAD" version 2>/dev/null | awk '{print $3}')
    if [[ -n "$DOWNLOADED_VER" && "$DOWNLOADED_VER" == "$CURRENT_VER" ]]; then
        _log_info "下载的版本 ($DOWNLOADED_VER) 与当前版本相同，无需更新。"
        rm -f "$TMP_HY_DOWNLOAD"
        return 0
    ...
    _log_info "准备替换 Hysteria 二进制文件..."
    _control_service "stop" # Stop service before replacing binary
    sleep 1
    if mv "$TMP_HY_DOWNLOAD" "$HYSTERIA_INSTALL_PATH"; then
        _log_success "Hysteria 已更新至 $DOWNLOADED_VER (或 latest)。"
        # Restore capabilities if they were set (important for ACME)
        if getcap "$HYSTERIA_INSTALL_PATH" | grep -q "cap_net_bind_service"; then
            _log_info "重新应用 setcap 权限..."
            if ! setcap 'cap_net_bind_service=+ep' "$HYSTERIA_INSTALL_PATH"; then
                 _log_warning "重新应用 setcap 失败。"
            fi
        fi
        _control_service "start" # Restart service
    else
        _log_error "替换 Hysteria 二进制文件失败。"
        rm -f "$TMP_HY_DOWNLOAD"
        _log_info "尝试重新启动旧的服务..."
        _control_service "start" # Try to restart old version if replacement failed
        return 1
    fi
}
...
_update_hy_script() {
    ...
    _setup_hy_command # This function already handles download/update
}
...
_show_menu() {
    ...
    echo " 安装与更新:"; echo "   install       - 安装或重新安装 Hysteria"; echo "   update        - 更新 Hysteria 程序 和 '${SCRIPT_COMMAND_NAME}' 脚本"
...
case "$ACTION" in
    ...
    update)          _do_update ;; # Added update command
    ...
    config_edit)
        _ensure_root; if ! _is_hysteria_installed; then _log_error "Hysteria未安装."; exit 1; fi; if [ -z "$EDITOR" ]; then EDITOR="vi"; fi
        _log_info "使用 $EDITOR 打开 $HYSTERIA_CONFIG_FILE ..."; if $EDITOR "$HYSTERIA_CONFIG_FILE"; then _log_info "编辑完成。考虑重启服务: sudo $SCRIPT_COMMAND_NAME restart"; else _log_error "编辑器 '$EDITOR' 返回错误。"; fi ;;
